...
lmgod1 <- lm( (個經PR) ~ (可修個體), data = hw1Data )
tidy(lmgod1)

```

調整standard error後的估計結果：
```{r, results='asis'}
lmgod01 <- coeftest(lmgod1, vcov. = vcovHC, type="HC1")
stargazer(lmgod01,
          se=list(lmgod01[,"Std. Error"]),
          type="html",
          align=TRUE)
```
...
lmgod2 <- lm( (個經PR) ~ (可修個體) + (經原PR), data = hw1Data)
lmgod02 <- coeftest(lmgod2, vcov. = vcovHC, type="HC1")
stargazer(lmgod02,
          se=list(lmgod02[,"Std. Error"]),
          type="html",
          align=TRUE)
...
假設$E(\epsilon_i|x_i)=0$，證明上題設定下：
  ...
  $$V(\hat{\beta}_1|X)=\hat{\beta}_1=\frac{\sum_{i=1}^N (x_i-\bar{x})(y_i)}{\sum_{i=1}^N (x_i-\bar{x})^2}$$
  $$=(1/\sum_{i=1}^N (x_i-\bar{x})^2)V(y_i|X)$$
  $$=(1/n_1-(n_1^2/n))\sigma^2$$
  $$(1/(n_1n-n_1^2/n))\sigma^2$$
  $$=(1/(n_1^2+n_0n_1-n_1^2/n)\sigma^2$$
  $$=((n_0+n_1)/n_0n_1)\sigma^2$$
  $$=(n/n_0n_1)\sigma^2$$
