...
library("knitr")
library(psych)
ols1=lm(個經PR~可修個體,data=hw1Data)
summary(ols)
library(sandwich)
library(lmtest)
library(stargazer)
ols1_coeftest=coeftest(ols1, vcov. = vcovHC, type="HC1")
stargazer(ols1, 
...
上題的估計結果很可能不正確，請說明原因故事（非數學證明）。
    ...
    立足點高的人（較聰明的人）就算沒有擋修制度，個經依然可以修得很好，所以上題的估計裡面的效果可能包含了原本學生資質的不同。
...
```{r ans16}
ols2=lm(個經PR~可修個體+經原PR,data=hw1Data)
summary(ols2)
ols2_coeftest=coeftest(ols2, vcov. = vcovHC, type="HC1")
stargazer(ols2, 
...
```{r ans17}
group=group_by(hw1Data,可修個體)
sum=summarise(group,
        meaneco=mean(經原PR,na.rm=T),
        meanmicroeco=mean(個經PR,na.rm=T))
head(sum)
