...
$$\hat{\beta_1}=\frac{\sum(x_i-\bar{x})(y_i-\bar{y})-<1>}{\sum(x_i-\bar{x})^2-<2>}$$

$$<1>{\sum(x_i-\bar{x})(y_i-\bar{y})}=\sum(x_i-\bar{x})y_i=\sum{x_iy_i}-\sum{\bar{x}y_i}$$
$$=n\bar{y_i}-\frac{n_1}{n_0+n_1}\sum{y_i}=n_1\bar{y_1}-\frac{n_1}{n_0+n_1}(n_0\bar{y_0}+n_1\bar{y_1})$$
$$=\frac{1}{n_0+n_1}[\frac{n_1(n_0+n_1)\bar{y_1}}{n_1^2\bar{y_1}+n_0n_1\bar{y_1}}-n_1^2\bar{y_1}-n_0n_1\bar{y_0}]$$
$$=\frac{n_0n_1}{n_0+n_1}(\bar{y_1}-\bar{y_0})$$

$$<2>\sum(x_i-\bar{x})^2=\sum{x_i}^2-n\bar{x}^2=n_1-n(\frac{n_1}{n_0+n_1})^2$$
$$=n_1-(n_0+n_1)\frac{n_1^2}{(n_0+n_1)^2}=n_1-\frac{n_1^2}{(n_0+n_1)}=\frac{n_1(n_0+n_1)-n_1^2}{n_0+n_1}$$
$$=\frac{n_0n_1}{n_0+n_1}$$

$$\hat{\beta_1}=\frac{<1>}{<2>}=\frac{\frac{n_0n_1}{n_0+n_1}(\bar{y_1}-\bar{y_0})}{\frac{n_0n_1}{n_0+n_1}}$$
...
$$=V(\frac{\sum(x_i-\bar{x})u_i}{\sum(x_i-\bar{x})}|x)=E[(\frac{\sum(x_i-\bar{x})u_i}{\sum(x_i-\bar{x})^2})^2|x]-[E(\frac{\sum(x_i-\bar{x})}{\sum(x_i-\bar{x})^2}|x)]^2$$
$$=E[(\frac{\sum(x_i-\bar{x})u_i}{\sum(x_i-\bar{x})^2})^2|x]=\frac{1}{[\sum(x_i-\bar{x})^2]^2}E[(\sum(x_i-\bar{x})u_i)^2|x]=\frac{1}{[\sum(x_i-\bar{x})^2]^2}\sum{E[(x_i-\bar{x})^2u_i^2|x]}$$
