...
```{r ans13}
model1HW <- lm((個經PR)~ (可修個體), data =hw1Data)
model1HW_coeftest<- coeftest(model1, vcov. = vcovHC, type="HC1")
#lm(("個經PR")~(可修個體)%>%(data=hw1Data))->mode11HW
#mode11HW<- lm((個經PR)~ (可修個體),data= hw1Data)
#coeftest(mode11HW, vcov. = vcovHC, type="HC1")->mode11HW_coeftest

summary(mode11HW)
summary(mode11HW_coeftest)
```
