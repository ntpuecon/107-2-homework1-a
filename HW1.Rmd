...
hw1Data_god <- hw1Data[,c("個經PR","可修個體")]
colnames(hw1Data_god) <- c("Y","X1")

lmgod <- lm(Y ~ X1, data = hw1Data_god)
tidy(lmgod)
...
```{r, results='asis'}
lmgod1 <- coeftest(lmgod, vcov. = vcovHC, type="HC1")
stargazer(lmgod1,
          se=list(lmgod1[,"Std. Error"]),
          type="html",
          align=TRUE)
```
