...
如果是文字答案，請直接在該題空白處回答。library("AER")
...
我們以個體經濟學為考量之進階課程，學生學習成效以**個經PR**（即學生第一次第一學期修完個經在同班的排名）來衡量。令**可修個體**虛擬變數其值為1若該學生通過修課限制可以修個經，反之為0。請寫下正確的效應結構（ 課堂上的Y即這裡的 **個經PR**，請對應課堂的$Y_{1i}-$Y_{1i}，寫法，寫出這題的表示句子。)

$Y_{1i}$為是可修習個經同學的pr，$Y_{1i}$不可修習個經同學pr，兩者相減是是否可以修習個經同學會對pr造成影響


## 1.2 效應結構
接續上題，課堂上的treatment dummy即為這裡的**可修個體**dummy，請寫下對應課堂效應結構$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})T_i$之對應寫法（以這裡的文字符號表示）。
$i$表示所有學生
$T_i$是dummy variable，$T_i=1$是有通過限制的學生，$T_i=0$是沒有通過限制的學生，所以當通過限制的學生$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})*1$其結果是$Y_{1i}$
，沒通過的學生是$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})*0$其結果是$Y_{0i}$
## 1.3 簡單迴歸模型
...
library(sandwich)
library(lmtest)
lm((個經PR)~ (經原PR),data=hw1Data)->model1HW

model1HW_coeftest<- coeftest(model1HW, vcov. = vcovHC, type="HC1")



summary(model1HW)
summary(model1HW_coeftest)
```

## 1.4 選擇性偏誤
上題的估計結果很可能不正確，請說明原因故事（非數學證明）。

立足點不同:當限制通過的學生可能會想要一次就過個經，就努力學習，考的成績就會比較好；反之，當限制不通過的學生，他可能覺得反正修了個經也不計分，所以就不認真，有可能成績就會比較低。

## 1.5 選擇性偏誤
這個資料還包含**經原PR**變數，它是學生最後修過的經原成績在該經原班的排名。說明有必要控制**經原PR**的理由（非數學證明）。

有可能因經原PR比較高的的分數，而個經成績好，即使微積分0分，所以如果控制經原PR，讓經原PR跟個經成績獨立，簡回歸會比較準確。
...
```{r ans16}
lm((個經PR)~ (可修個體)+(經原PR),data=hw1Data)->model2HW
model2HW_coeftest<-coeftest(model2HW, vcov. = vcovHC, type="HC1")

summary(model2HW)
summary(model2HW_coeftest)
```
...
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)__下降__(上升/下降) (2)__6.802__百分點，其值比未控制**經原PR**時還 (3)__高__(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)_正向偏誤___(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)__正向__(正向/反向)關連
  2. **可修個體**=1的族群，其**經原PR**平均較(6)__高__(高/低)
  
故若不控制**經原PR**，因**可修個體**=1與=0兩群人也會分別帶有 **經原PR**  (7)__高與低__(高與低/低與高)特質，所以**可修個體**=1的人其**個經PR**高，有部份是因其**經原PR**也較(8)_高  ___(高/低)所至。
...
$$\bar{y}_1=\sum_{i,x_i=1}y_i/n_1$$與
$$\bar{y}_0=\sum_{i,x_i=0}y_i/n_0$$

分別為$x_i=1$與$x_i=0$兩群樣本的$y_i$平均。
...
$$= \frac { \sum_{i=1}^n (x_i-\bar{x})^2 V(y_i|x) } { (\sum_{i=1}^n (x_i-\bar{x})^2)^2 }$$
$$= \frac { \sum_{i=1}^n (x_i-\bar{x})^2 V(\beta_0+\beta_1x_i+\epsilon_i|x) } { (\sum_{i=1}^n (x_i-\bar{x})^2)^2 }$$
$$= \frac { \sum_{i=1}^n (x_i-\bar{x})^2 V(\epsilon_i|x) } { (\sum_{i=1}^n (x_i-\bar{x})^2)^2 }$$
...
## 2.3 三明治表示式
線性迴歸模型
$$Y=X\beta+\epsilon$$的OLS估計式$$\hat{\beta}=(X'X)^{-1}(X'Y)$$
。條件在已知$X$下，令
$$V(\epsilon|X)=\Sigma X(X'X)^{-1}$$

。請證明
$$V(\hat{\beta}|X)=(X'X)^{-1}X'\Sigma X(X'X)^{-1}$$
，具有三明治形式（即可表示成$BMB'$的矩陣相乘形式）。
