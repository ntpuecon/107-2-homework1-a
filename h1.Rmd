...
$Y_{1i}-Y_{0i}$
$Y_{1i}$為通過限制可修個經的學生PR，$Y_{0i}$為未通過限制之學生的PR，
故兩者相減可表示是否通過限制對學生PR的影響。

## 1.2 效應結構
接續上題，課堂上的treatment dummy即為這裡的**可修個體**dummy，請寫下對應課堂效應結構$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})T_i$之對應寫法（以這裡的文字符號表示）。

$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})T_i$
$Y_i$為學生的個經PR，$Y_{0i}$為未通過限制學生的PR，$T_i$則是treatment dummy若學生有通過限制則$T_i=1$，
故若學生有通過個經限制，則$(Y_{1i}-Y_{0i})*1$為有通過限制的效果則會顯現。
...
假設$E(\epsilon_i|x_i)=0$，證明上題設定下：
  ...
  (b) 若條件在已知每個觀測值的$x_i$為1或0下$V(\epsilon_i|x_i)=\sigma^2$（即齊質變異），則條件變異數$$V(\hat{\beta}_1|X)=\frac{n}{n_1 n_0}\sigma^2$$。
